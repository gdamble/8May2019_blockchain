                                    ...
                                    if ((result.toString() != "\"\"") && ("1234567" == JSON.parse(JSON.parse(result.toString()))['optedServices'])) {
                                        ...
                                        if ([...optedSrvcs2].every(val => [...optedSrvcs1].includes(val))) {
                                            ...
                                            let intersection = new Set(
                                                [...optedSrvcs1].filter(x => (new Set([...optedSrvcs2])).has(x)));

                                            if (intersection.size != 0) {
                                                console.log('The number is already Partially Registered for :', numToString(sortString([...intersection].join(''))));
                                            }
                                ...
                                case 3:
                                    ...
                                    if ((diffSecondsTillNow(JSON.parse(JSON.parse(result.toString()))['activationdate']) < 90) && (JSON.parse(JSON.parse(result.toString()))['status'] == "Active")) {
                                        ...
                                        if (eqSet(difference1, new Set([...optedSrvcs1]))) {
                                            ...
                                            var optedSrvcs1Set = new Set([...optedSrvcs1]);
                                            var difference2 = new Set([...optedSrvcs2].filter(x => !optedSrvcs1Set.has(x)));
                                            if (difference2.size != 0) {
                                                console.log('This Mobile Number is not Registered for DND options: ', numToString(sortString([...new Set([...difference2])].join(''))));
                                            }
